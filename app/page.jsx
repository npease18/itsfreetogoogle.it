...
export default function Home() {
  const [websiteUrl, setWebsiteUrl] = useState(""); // Default URL, can be changed later
  const [canSeeWebsite, setCanSeeWebsite] = useState(false);
  var { register, control, reset, handleSubmit } = useForm();

  function generateWebsiteUrl(data) {
  ...
  return (
    <div className="font-sans grid items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20">
      <div className="items-center justify-items-center w-[100%] text-center">
        ...
        <div className="mt-5 max-md:w-[100%] md:w-[45%]">
          <Form
            onSubmit={handleSubmit(generateWebsiteUrl)}
            control={control}
          >
          <input
            type="text"
            placeholder="What seems to be the struggle to Google?"
            className="p-2 border border-gray-300 rounded-md focus:outline-none mr-2 w-[100%] text-center"
            {...register("query")}
          />
          </Form>
        </div>
